    ...
    <main className="flex flex-1 flex-col overflow-hidden bg-background">
      <header className="border-b border-divider px-4 py-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-text-primary">외부 멤버</h2>
          <button
            onClick={() => setShowInviteForm(prev => !prev)}
            className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-on-primary transition-colors hover:bg-[var(--color-state-primary-pressed)]"
          >
      ...
      <div className="scrollbar-thin flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-8">
            <span className="text-sm text-text-tertiary">외부 멤버가 없습니다</span>
          </div>
        ) : (
          members.map(member => (
    ...
    <div className="border-b border-divider bg-surface px-4 py-3">
      <div className="space-y-2">
        <input
          ...
          className="w-full rounded-lg border border-divider bg-background px-3 py-2 text-sm text-text-primary outline-none focus:border-primary"
        />
        <input
          type="text"
          value={name}
        ...
        <button
          ...
          className="w-full rounded-lg bg-primary py-2 text-sm font-semibold text-on-primary disabled:bg-disabled"
        >
    ...
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      {member.inviteStatus === 'PENDING' && (
        <button
          ...
          disabled={isPending}
          className="shrink-0 rounded-lg border border-red-200 px-2.5 py-1.5 text-xs text-red-500 hover:bg-red-50 disabled:opacity-50"
        >
          {isPending ? '취소중' : '취소'}
        </button>
      )}
