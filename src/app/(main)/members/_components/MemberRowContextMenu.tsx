...
/**
 * 멤버 행 우클릭 메뉴 (데스크톱 관례) — [프로필 보기 / 관심멤버 지정·해제 / 차단하기].
 * 액션 규칙·컨펌·스낵바는 프로필 화면(UserProfileDialog)과 동일:
 * 본인 행은 프로필만, 관심멤버는 게스트 뷰어 제외, 차단은 정책(isBlockableMember) 충족 시.
 */
export function MemberRowContextMenu({ member, x, y, onOpenProfile, onClose }: MemberRowContextMenuProps) {
  ...
  const handleBlockConfirm = () => {
    blockMember(member, {
      onSuccess: () => {
        showSnackbar({
          message: member.isExternal ? BLOCK_MESSAGES.blockManageHint : BLOCK_MESSAGES.blocked,
        });
      },
    });
    onClose();
