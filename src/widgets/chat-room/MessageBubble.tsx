...
export function MessageBubble({
  ...
  const hasContextMenu =
    !isDeleted && !isSystem && !isFailed && !isBlockedCollapsed && !isReportedMask &&
    ...
    >
      ...
      >
        ...
        <div className={cn('flex items-end gap-1', isMe ? 'flex-row-reverse' : 'flex-row')}>
          ...
          <div
              className={cn(
                ...
                isFileMessage && fileDownloadUrl && !isBlockedCollapsed && 'self-stretch justify-between',
              )}
            >
              {message.isLocal ? (
                isFailed ? (
                ...
                <>
                  {isFileMessage && fileDownloadUrl && !isBlockedCollapsed && (
                    <a
        ...
        {hasTags && (
          <BubbleTagRow
            tags={message.tags!}
            // 차단 발신자 메시지의 태그는 표시만 — 편집 진입 차단 (RN 패리티)
            onClick={onEditTag && !isBlocked ? () => onEditTag(message) : undefined}
            className="mt-1"
          />
        )}
      </div>
  ...
  return (
    <>
      ...
      <MessageContextMenu
        ...
        onDelete={onDeleteMessage && isWithinDeleteWindow && !otherUserIsRemoved ? () => onDeleteMessage(message.id) : undefined}
        onReport={!isMe && onReportMessage && !otherUserIsRemoved ? () => onReportMessage(message.id) : undefined}
        quickTags={quickTags}
        onMoreTags={!isBlocked && !otherUserIsRemoved && onEditTag ? () => onEditTag(message) : undefined}
      >
        {bubbleContent}
      </MessageContextMenu>
    </>
